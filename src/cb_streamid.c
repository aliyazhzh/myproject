...
int parse_vlan_tag(sr_session_ctx_t *session, sr_val_t *value, uint8_t *vlan)
{
	...
	if (!strcmp(vlan_str, "tagged")) {
		...
		*vlan = 3;
		...
		sr_session_set_error_message(session, err_msg, value->xpath);
...
int get_streamid_per_port_per_id(sr_session_ctx_t *session, const char *path)
{
	...
	if (rc != SR_ERR_OK) {
		...
		sr_session_set_error_message(session, err_msg, path);
...
int parse_streamid_per_port_per_id(sr_session_ctx_t *session, bool abort)
{
	...
	while (cur_node) {
		...
		if (rc == SR_ERR_NOT_FOUND) {
			...
			sr_session_set_error_message(session, err_msg, xpath);
			printf("ERROR: %s sr_get_items: %s\n", __func__,
			       sr_strerror(rc));
 ...
 *
 ************************************************************************/
int cb_streamid_subtree_change_cb(sr_session_ctx_t *session, uint32_t sub_id, const char *module_name, const char *path,
